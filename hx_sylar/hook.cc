...
namespace hx_sylar {
	static hx_sylar::ConfigVar<int>::ptr g_tcp_connect_timeout =
			hx_sylar::Config::Lookup("tpc.connect.timeout",500,"tcp connect timeout");
    ...
    if (to != static_cast<uint64_t>(-1)) {
      timer = iom->addConditionTimer(
          ...
          [winfo, fd, iom, event]() {
            ...
            iom->cancelEvent(fd,
                             static_cast<hx_sylar::IOManager::Event>(event));
          },
          winfo);
    }
    int rt = iom->addEvent(fd, static_cast<hx_sylar::IOManager::Event>(event));
    if (rt) {
      if (timer) {
        timer->cancel();
      }
...
auto connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen)
    -> int {
...
auto accept(int s, struct sockaddr *addr, socklen_t *addrlen) -> int {
...
auto readv(int fd, struct iovec *iov, int iovcnt) -> ssize_t {
...
auto recv(int sockfd, void *buf, size_t len, int flags) -> ssize_t {
  return do_io(sockfd, recv_f, "recv", hx_sylar::IOManager::READ, SO_REUSEADDR,
               buf, len, flags);
}
...
auto write(int fd, const void *buf, size_t count) -> ssize_t {
  return do_io(fd, write_f, "write", hx_sylar::IOManager::WRITE, SO_SNDTIMEO, buf,
        count);
}
...
auto ioctl(int d, unsigned long request, ...) -> int {
  va_list va;
  va_start(va, request);
  ...
  if (FIONBIO == request) {
    bool user_nonblock = *(int *)arg != 0; // !!*(int*)(arg)
...
auto getsockopt(int sockfd, int level, int optname, void *optval, socklen_t *optlen) -> int {
...
auto setsockopt(int sockfd, int level, int optname, const void *optval,
  ...
  if (level == SOL_SOCKET) {
    if (optname == SO_RCVTIMEO || SO_SNDTIMEO == optname) {
