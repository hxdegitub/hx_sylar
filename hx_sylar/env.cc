  ...
  for (int i = 1; i < argc; ++i) {
    if (argv[i][0] == '-') {
      if (strlen(argv[i]) > 1) {
        ...
        HX_LOG_ERROR(g_logger) << "invalid arg idx=" << i << " val=" << argv[i];
        return false;
      }
    } else {
      if (now_key) {
...
std::string Env::getAbsoluteWorkPath(const std::string& path) const {
  ...
  static hx_sylar::ConfigVar<std::string>::ptr g_server_work_path =
      hx_sylar::Config::Lookup<std::string>("server.work_path");
